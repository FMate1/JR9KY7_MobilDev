...
class CartActivity : ComponentActivity() , View.OnClickListener, CartItemClickListener {
    ...
    private lateinit var myDb : ShoesDB
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        myDb = Room.databaseBuilder(this, ShoesDB::class.java, "myApp2")
    ...
    private fun getDataFromDb(){
        CoroutineScope(Dispatchers.IO).launch {
            this.let {
                var cat = ShoesDB.getDatabase(this@CartActivity).getCartDao().getAllCartItems()
    ...
    override fun onItemDelete(cart: Cart){
        ...
        totalSum -= cart.quantity * cart.shoesAdded.price
    ...
    private fun setBottomSum(){
            CoroutineScope(Dispatchers.IO).launch {
                this.let {
                    var cat =
                        ShoesDB.getDatabase(this@CartActivity).getCartDao().getAllCartItems()
                    arrCarts = cat as ArrayList<Cart>
                    ...
                    if (arrCarts.size <= 0) {
                        ...
                        for (cart: Cart in arrCarts) {
                            ...
                            if (cart.quantity == 1) {
                                sum += cart.shoesAdded.price
                                quantity++
                            } else if (cart.quantity > 1) {
                                sum += cart.shoesAdded.price * cart.quantity
